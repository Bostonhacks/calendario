...
function cleanClasses (calendarId) {
  ...
  return deleteClasses(calendarId)
...
function createClasses (calendarId) {
  ...
  for (const day of Object.keys(createDays)) {
    classArray.push(createEvent(calendarId, createDays[day], day))
  }

  return Promise.all(classArray)
}

function createEvent (calendarId, day, name) {
  return new Promise((resolve, reject) => {
    let start = moment.tz(day.startDate, 'MM-DD-YYYY', tz)
    let end = moment.tz(day.endDate, 'MM-DD-YYYY', tz)
    ...
    calendarAPI.events.insert({
      ...
      if (err) {
        debug('Error on inserting special day: ' + err)
        return reject(err)
      }
      resolve()
    })
